...
func startTraining(charName, charClass string) (string, error) {
	if charClass == "warrior" {
	...
	return "тренировка окончена", nil

}

// обратите внимание на имя функции и имена переменных
func choiceCharClass() (string, error) {
...
func main() {
	...
	charClass, err := choiceCharClass()
	if err != nil {
		fmt.Println(err)
	}
	training, err := startTraining(charName, charClass)
	if err != nil {
		fmt.Println(err)
	}
